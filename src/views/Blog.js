...
function Blog(){
    ...
    //when component "mounts" (aka loads), get external apu data using Fetch
    useEffect(()=>{
            ...
            <ul className="list-group">
                {blogPosts.map((post) => (
                    <li className="list-group-item" key={post.postId}>
                        ...
                        <div>
                            Posted : 
                            <span className="badge text-bg-secondary m-1">{new Date(post.date).toLocaleDateString()}</span>
                        </div>
                    </li>
                ))}
            </ul>
